...
# Project from https://github.com/JPKP-Kuhn/PCIConcursos
class Scrap_csv:
    """ Cria um arquivo csv com os concursos de cada estado """

    def exam_region(self, source_code, region):
    ...
    def new_exam(self, user_dir: str, date_now: str):
        """Verifica se há novos concursos comparando com arquivo anterior.
        
        Args:
            user_dir: Diretório do usuário onde estão os arquivos CSV
            date_now: Data no formato ddmmyyyy
        """
        novos_concursos = ['Concursos novos disponíveis: ']
        csv_file = os.path.join(user_dir, 'ConcursosAtivos.csv')
        csv_file_today = os.path.join(user_dir, f'ConcursosAtivos{date_now}.csv')

        # Check if there is an old 'ConcursosAtivos.csv' file
        if not os.path.isfile(csv_file):
            if os.path.isfile(csv_file_today):
                os.rename(csv_file_today, csv_file)
        else:
            if os.path.isfile(csv_file_today) and not filecmp.cmp(csv_file, csv_file_today):
                antigo = pd.read_csv(csv_file, encoding='utf-16', header=None, sep="\t")
                novo = pd.read_csv(csv_file_today, encoding='utf-16', header=None, sep="\t")
    ...
    def run(self, user_id: int, state: str, LINK="https://www.pciconcursos.com.br/concursos/"):
        """Executa o scraping e salva o CSV no diretório do usuário.
        
        Args:
            user_id: ID do usuário do Telegram, para criar o arquivo
            state: Sigla do estado (ex: 'SP', 'RJ', 'MG')
            LINK: URL base do site PCI Concursos
        """
        # Date
        today = date.today()
        ...
        # Extract state data (state is the abbreviation like 'SP', 'RJ', etc.)
        state_data = self.exam_region(soup, state)

        # Save as CSV
        df = pd.DataFrame(state_data)
        df = df.replace(r'^\s*$', '-', regex=True)
        csv_file_today = os.path.join(user_dir, f'{user_id}_{date_now}.csv')
        
        with open(csv_file_today, 'w', encoding='utf-16', newline='') as f:
            df.to_csv(f, encoding='utf-16', header=False, sep="\t", index=False)

        # Check for new exam
        self.new_exam(user_dir, date_now)
